      ...
      >
        ...
        <el-table-column prop="icon" label="图标" width="80" align="center">
          <template #default="scope">
            <el-icon v-if="isValidComponent(scope.row.icon)">
              <component :is="scope.row.icon" />
            </el-icon>
            <span v-else-if="scope.row.icon" style="color: #F56C6C; font-size: 12px; font-weight: bold;" title="图标名称不合法">
              异常
            </span>
          </template>
        </el-table-column>
...
// 🌟 图标选择器按需加载 — 67 个图标组件只在打开菜单弹窗时才拉取
...
const validatePath = (rule: any, value: string, callback: any) => {
  if (!value) return callback(new Error('请输入路由路径'));
  
  if (!form.value.parentId) {
    if (!value.startsWith('/')) {
      return callback(new Error('顶级菜单的路径必须以 "/" 开头，如: /system'));
    }
  } else {
    if (value.startsWith('/')) {
      return callback(new Error('子菜单路径不能以 "/" 开头，请直接输入字母，如: user'));
    }
  }
  callback();
};
...
const validateComponent = (rule: any, value: string, callback: any) => {
  ...
  if (/^\d+$/.test(value)) {
    return callback(new Error('组件路径不能是纯数字，请填写 Layout 或正确路径'));
  }
  
  if (value !== 'Layout' && value.startsWith('/')) {
    return callback(new Error('组件路径不能以 "/" 开头，请去掉斜杠'));
  }
...
const validateName = (rule: any, value: string, callback: any) => {
  if (!value) return callback(new Error('请输入路由名称'));
  
  if (!/^[a-zA-Z][a-zA-Z0-9]*$/.test(value)) {
    return callback(new Error('路由名称必须以英文字母开头，且只能包含字母和数字'));
  }
  callback();
};
...
const validateIcon = (rule: any, value: string, callback: any) => {
  if (value && /^\d+$/.test(value)) {
    return callback(new Error('图标不能是纯数字，请输入有效的 Element Plus 图标名'));
  }
  callback();
};
...
const handleDelete = (row: any) => {
  ElMessageBox.confirm(`确认删除菜单 "${row.title}"？（若有子菜单将一同删除）`, '警告', { type: 'warning' })
    .then(async () => {
